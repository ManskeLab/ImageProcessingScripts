...
def plot(comb: tuple, directory: str) -> None:
  ...
  plt.title("%s - %s" % (scanToName[index1], scanToName[index2]))
  plt.xlabel("Mean (%s and %s)" % (scanToName[index1], scanToName[index2]))
  plt.ylabel("Error (%s - %s)" % (scanToName[index1], scanToName[index2]))
  plt.ylim(-70,70)
  plt.plot(df["mean100"], df["difference100"])
  plt.plot(df["mean400"], df["difference400"])
  plt.plot(df["mean800"], df["difference800"])
